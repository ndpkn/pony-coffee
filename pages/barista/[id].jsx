...
const BaristaPage = () => {
	...
	useEffect(() => {
		...
		ponyService.getBaristaPage(pid).then(onBaristaLoaded).catch(onError)
	}, [pid])
	...
	const onBaristaLoaded = baristaList => {
		...
		setCoffeePots(
			baristaList.coffeePots,
			baristaList.coffeePots.unshift({
				address: 'Выберите место работы',
				id: 0,
			})
		)
		setUserCoffeePotId(
			baristaList.user.user_coffee_pot
				? baristaList.user.user_coffee_pot.coffee_pot_id
				: null
		)
		setLoading(false)
	}
