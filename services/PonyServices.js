...
class PonyService {
	...
	addNotification = async data => {
		const res = await this.postData(
			`${this._apiBase}/admin/notification`,
			data
		)
		return res
	}
	...
	createUser = async data => {
		const res = await this.postData(`${this._apiBase}/user/create`, data)
		return res
	}
	getCode = async data => {
	...
	getEmailCode = async data => {
		const res = await this.postData(
			`${this._apiBase}/mail/verification/code`,
			data
		)
		return res
	}
	...
	addMessageAdmin = async (data, id) => {
		const res = await this.postData(
			`${this._apiBase}/admin/feedback/${id}`,
			data
		)
		return res
	}
	...
	changeCoffeePot = async (data, id) => {
		const res = await this.putData(
			`${this._apiBase}/admin/coffeePot/${id}`,
			data
		)
		return res
	}
	...
	changePass = async data => {
		const res = await this.putData(
			`${this._apiBase}/profile/password`,
			data
		)
		return res
	}
